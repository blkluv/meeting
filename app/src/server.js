...
const api_key_secret = process.env.API_KEY_SECRET || "MeetONS_default_secret";
...
app.post([apiBasePath + "/meeting"], (req, res) => {
  ...
  if (authorization != api_key_secret) {
    log.debug("MeetONS get meeting - Unauthorized", {
      header: req.headers,
      body: req.body,
    });
  ...
  log.debug("MeetONS get meeting - Authorized", {
    header: req.headers,
    body: req.body,
    meeting: meetingURL,
  });
